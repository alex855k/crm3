$(document).ready(() => {
    ...
    function filterInputDelay(caller) {
        clearTimeout($.data(this, 'timer'));
        $(this).data('timer', setTimeout(Search(true, null), 500));
    }
    ...
    function Search(forceSearch, pageNumber) {
        ...
        searchObject.CustomerId = $("#CustomerId").val();
        var filterRow = $("#filterTr").clone();
        $.ajax({
            url: "/EmailMessages/DynamicCorrespondenceTable",
            data: { "emailMessageViewModel": searchObject },
            type: "POST",
            success: function (data, textStatus, jqXHR) {
                debugger;
                $("#emailMessagesListDiv").html(data.tablePartial);
                $("#emailMessagePaginationDiv").html(data.pagingPartial);
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                toastr.error("failed load email correspondance");
            }
        });
    }
})